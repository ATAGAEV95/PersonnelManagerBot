...
async def flood(message: Message) -> None:
    """Обрабатывает все сообщения, не перехваченные другими хэндлерами (fallback).

    Функция выступает в роли последнего обработчика сообщений и активируется,
    когда пользователь отправляет текст, не соответствующий командам или
    ожидаемым форматам ввода бота. Перенаправляет пользователя к использованию
    меню бота вместо произвольных текстовых сообщений.
    """
